...
advance :: Scanner ()
advance = modify (\ScannerState{..} -> ScannerState{current = current + 1, ..})
...
scanner source = let result = evalState (scanner' source) (ScannerState 0 1) in
...
scanner' source = case (T.head source, T.tail source) of
    ('(', t) -> advance *> mktok LeftParen <*> scanner' t
    (')', t) -> advance *> mktok RightParen <*> scanner' t
    ('{', t) -> advance *> mktok LeftBrace <*> scanner' t
    ('}', t) -> advance *> mktok RightBrace <*> scanner' t
    (',', t) -> advance *> mktok Comma <*> scanner' t
    ('.', t) -> advance *> mktok Dot <*> scanner' t
    ('-', t) -> advance *> mktok Minus <*> scanner' t
    ('+', t) -> advance *> mktok Plus <*> scanner' t
    (';', t) -> advance *> mktok Semicolon <*> scanner' t
    ('*', t) -> advance *> mktok Star <*> scanner' t
    ('!', t) -> advance *> (case T.head t of
        '=' -> advance *> mktok BangEqual <*> scanner' (T.tail t)
        _ -> mktok Bang <*> scanner' t)
    ('=', t) -> advance *> (case T.head t of
        '=' -> advance *> mktok EqualEqual <*> scanner' (T.tail t)
        _ -> mktok Equal <*> scanner' t)
    ('<', t) -> advance *> (case T.head t of
        '=' -> advance *> mktok LessEqual <*> scanner' (T.tail t)
        _ -> mktok Less <*> scanner' t)
    ('>', t) -> advance *> (case T.head t of
        '=' -> advance *> mktok GreaterEqual <*> scanner' (T.tail t)
        _ -> mktok Greater <*> scanner' t)
    ('/', t) -> advance *> (case T.head t of
        '/' -> scanner' $ T.dropWhile (/= '\n') t
        _ -> mktok Slash <*> scanner' t)
    (' ', t) -> advance *> scanner' t
    ('\r', t) -> advance *> scanner' t
    ('\t', t) -> advance *> scanner' t
    ('\n', t) -> newline *> scanner' t
    ('"', t) -> do
        (res, t') <- string t
...
string source = do
    ...
    if T.null t then
        pure (Left $ ScanErr line "Unterminated string" current, t)
    else do
        advance
...
number source = do
    ...
    ScannerState{..} <- get
    if T.null t || T.head t /= '.' then do
        let newCurrent = current + T.length num
        put $ ScannerState{current = newCurrent, ..}
        pure (Right $ Token (Number . read $ T.unpack num) line current, t)
    else do
        let (num', t') = T.span isDigit $ T.tail t
        let newCurrent = current + T.length num + T.length num' + 1
        put $ ScannerState{current = newCurrent, ..}
